/**
 * apiClient.ts — Customer App HTTP Client
 *
...
export interface ApiClientOptions extends Omit<RequestInit, 'method' | 'body'> {
  method?: HttpMethod;
  body?: unknown;
  /** Skip auth header injection (e.g. login / register endpoints). */
  skipAuth?: boolean;
}
 ...
 *
 * @param endpoint  API path relative to VITE_API_BASE_URL (e.g. '/api/v1/properties')
...
export async function apiClient<T = unknown>(
  ...
  // Only truly public auth endpoints skip the Bearer token.
  // /auth/me needs the token to identify the user.
  // /auth/refresh sends the refresh token in the body (not Bearer), so skipAuth is fine.
...
// ---------------------------------------------------------------------------
// ApiError — carries HTTP status code for consumers to branch on
// ---------------------------------------------------------------------------
